1. # Binary Compatible Critical Section Delegation
   ...
   2. If your CPU core count is not 32, please modify the `core_per_socket` variable in `micro_benchmark.c`, `test_latency.c`, and `rwlock.c`.
   3. Executing the script `test_thread_1.sh` yields Figure 5a.
   4. Executing the script `test_thread_2.sh` yields Figure 5b.
   5. Executing the script `test_thread_4.sh` yields Figure 5c.
   6. Executing the script `test_latency_90.sh` yields Figure 5d.
   7. Executing the script `test_latency_99.sh` yields Figure 5e.
   8. Executing the script `test_ncs.sh` yields Figure 5f. By default, we tested with 64 threads. You can modify the `tnum` variable to change the number of threads.
   9. Executing the script `test_cache_array.sh` yields Figure 5g. By default, we tested with 64 threads. You can modify the `tnum` variable to change the number of threads.
   10. Executing the script `test_cache_list.sh` yields Figure 5h. By default, we tested with 64 threads. You can modify the `tnum` variable to change the number of threads.
   11. Executing the script `test_rwlock.sh` yields Figure 5i. By default, we tested with 64 threads. You can modify the `tnum` variable to change the number of threads.
   
   ### Effect of Optimization (Paper Section 4.3)
   
   1. Enter the **benchmark/micro_benchmark** directory.
   2. If your CPU core count is not 32, please modify the `core_per_socket` variable in `micro_benchmark.c`.
   3. Executing the script `test_opt.sh` yields Figure 5j.
   ...
   3. By default, we tested the performance with 64 threads. You can modify the number of threads by changing the `thread_numer` parameter in the scripts according to your needs.
   4. Execute the scripts: `test_raytrace_teapot.sh` ,`test_raytrace_balls4.sh` and ` test_raytrace_car.sh` , to test the Raytrace app.
   5. Execute the script `test_radiosity.sh` to test the Radiosity app.
   6. Execute the script `test_volrend.sh` to test the Volrend app.
   7. Execute the script `test_ocean_cp.sh` to test the Ocean_cp app.
   ...
   8. `./configure`
   9. `make`
   10. By default, we use the last 8 cores to run 8 threads for the client, and the other cores to run memcached. You can modify the number of client threads by changing the `client_thread` variable in `pre.sh` and `test_one.sh`. Additionally, you can control the number of memcached threads by adjusting the array iterated by the for loop in `test_memcached.sh`.
   11. Executing the script `test_memcached.sh` yields Figure 5k.
   ...
   6. You can control the number of threads by adjusting the array iterated by the for loop in `test_leveldb.sh`.
   7. Executing the script `test_leveldb.sh` yields Figure 5l.