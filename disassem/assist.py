...
ops_def_flags = ['CMP', 'TEST', 
            ...
            'CLC', 'CLD', 'STC', 'STD','BT','LOCK CMPXCHG']

ops_use_flags = conditional_branch + ['LAHF', 'SET', 'CMOV','SBB']
...
def check_is_branch(lines):
    if len(lines)==0:
        return False
    name = get_inst_name(lines[-1])
    if check_in_list(name, conditional_branch) or check_in_list(name, unconditional_branch):
        return True
    return False
