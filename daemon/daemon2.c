...
const unsigned long targets[] = { 0x7ffff7fad170,0x7ffff7fa7170 ,0x7ffff7fa9170 };
...
void compile(int pid, int tgid, void* entry) {
    char cmd[1024];
    ...
    sprintf(cmd, "cd ./%p/%d/ && python3 ../../../disassem/disassem.py %d %p",
        entry, tgid, pid, entry);
    system(cmd);
}
...
void write_sys(int pid, int tgid, unsigned long RIP) {
    int rest_len;
    int to_copy_len;
    char i_buf[4096 * 256];
    char o_buf[4096 * 2];
    ...
    sprintf(jit_path, "./%p/%d/asm/jit.bin", (void*)RIP, tgid);
...
void check() {
    ...
    for (int i = 0;i < len / sz;i++, exe++) {
        ...
        if (exe->status_code == un_init) {
            sprintf(cmd, "mkdir -p ./%p/%d && cp -rf ./asm/ ./%p/%d", (void*)exe->RIP, exe->tgid, (void*)exe->RIP, exe->tgid);
            // sprintf(cmd, "true");
            ...
            if (valid_address(exe->tgt_addr) && valid_address(exe->RIP)) {
                sprintf(cmd, "echo %p >> ./%p/%d/asm/callee_table.txt", (void*)exe->tgt_addr, (void*)exe->RIP, exe->tgid);
            ...
            if (valid_address(exe->jump_from) && valid_address(exe->tgt_addr) && valid_address(exe->RIP)) {
                sprintf(cmd, "echo '%p %p' >> ./%p/%d/asm/ij_table.txt", (void*)exe->jump_from, (void*)exe->tgt_addr, (void*)exe->RIP, exe->tgid);
        ...
        if (cmd[0] && !cmd_history(cmd)) {
            printf(" Compiling");
            system(cmd);
            compile(exe->pid, exe->tgid, (void*)exe->RIP);
        }
        else {
            if (cmd[0]) {
        ...
        if (cmd[0]) {
            write_sys(exe->pid, exe->tgid, exe->RIP);
            printf("\n");
        }
        // if (is_valid == 0)
