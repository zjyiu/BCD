...
ops_new = load_numbers("./mutex/latency/90/new.txt")
ops_old = load_numbers("./mutex/latency/90/old.txt")
ops_tcs= load_numbers("./mutex/latency/90/tcs.txt") 
ops_tcb= load_numbers("./mutex/latency/90/tcb.txt") 
...
plt.axhline(y=2e6, color='grey', linestyle='dotted', linewidth=0.1)
for i in range(1,10):
    for j in range(2,6):
...
plt.savefig('mutex/latency/90/result.pdf', format='pdf')
plt.show()
